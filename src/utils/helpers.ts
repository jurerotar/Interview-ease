...
export const shareByEmail = (content: string): void => {
  const element = document.createElement('a');
  element.setAttribute('href', `mailto:?body=${encodeURIComponent(content)}`);
    ...
    .map((topicBlock: Element) => {
      ...
      const questions = Array.from(questionBlocks)
        .map((questionBlock: Element) => {
          ...
          const ratingElement = questionBlock.querySelector('[data-rating]');
          if (ratingElement) {
            rating = parseInt(ratingElement.getAttribute('data-rating') ?? '0', 10);
          }
        ...
        .filter((data) => (data.rating !== null && data.rating !== 0) || data.notes.length !== 0) as ParsedQuestion[];
